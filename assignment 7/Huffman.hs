...
huffman = getTree . combineTwo . sorthuff . convert

-- convert  a list of frequency-character pairs into a list of frequency-tree pairs, mapping each character to a Tip;
convert :: [(a,Int)] -> [(Int,Btree a)]
convert = map (\(x,y)->(y,Tip x))

-- sort only by x in (x,y) pairs
sorthuff :: [(Int,Btree a)] -> [(Int,Btree a)]
sorthuff = sortBy (\(x,_) (y,_) -> compare x y)

combineTwo :: [(Int,Btree a)] -> [(Int,Btree a)]
-- if a singleton set remains, return it
combineTwo [x] = [x]
-- otherwise repeatedly combine the first two elements of the list
combineTwo ((x,tx):(y,ty):xs) = combineTwo $ sorthuff $ (x+y,Bin tx ty):xs

getTree :: [(Int,Btree a)] -> Btree a
getTree [(x,y)] = y

-----------------------------------------------------------------------
codes :: Btree a -> [(a, [Bit])]
codes (Tip x) = [(x,[])]
codes (Bin l r) = map (\(x,y)->(x,O:y)) (codes l) ++ map (\(x,y)->(x,I:y)) (codes r)

-----------------------------------------------------------------------

encode :: (Ord a) => Btree a -> [a] -> [Bit]
-- idea: for x in xs, find the code for x in the list of codes for the tree, and append it to the result
encode tree xs = concat $ map (\x->snd $ head $ filter (\(y,_)->y==x) (codes tree)) xs

-----------------------------------------------------------------------

decode :: (Ord a) => Btree a -> [Bit] -> [a]
decode tree codes = decode' tree codes
  where
    decode' (Tip x) [] = [x]
    decode' (Tip x) xs = x:(decode' tree xs)
    decode' (Bin l r) (x:xs) = if x==O then decode' l xs else decode' r xs
